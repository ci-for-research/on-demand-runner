...
module.exports.check_run = async event => {
  ...
  const is_public_repo = !action.repository.private
  ...
  let is_external_pr = is_public_repo && !is_push && !is_internal_pr

  if (block_external_prs && is_external_pr) {
    console.log('External PR detected please cancel job');
    // TODO also do action
  }
